```bash
[rank0]:RuntimeError: aten._foreach_norm.Scalar: got mixed torch.Tensor and DTensor,
...
GPU0 GPU1 / GPU2 GPU3。假设 batch 中有两个样本 A 和 B，其中 GPU0、GPU1 负责样本 A，GPU2、GPU3 负责样本 B。对于样本 A，GPU0 和 GPU1 会一起计算同一个样本的一层，只是各自负责其中一部分计算，然后通过通信合成完整结果，因此同一行构成一个 TP group；而 GPU0 和 GPU2 虽然执行的是同一个模型位置上的计算，但处理的是不同样本，前向和反向基本独立，只需要在反向后同步梯度，因此同一列构成一个 DP group。
